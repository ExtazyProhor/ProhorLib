...
public interface AbstractGraph<T> {
    ...
    default boolean containsAllVertices(Collection<T> values) {
        for (T t : values) if (!containsVertex(t)) return false;
        return true;
    }

    default boolean containsAtLeastOneVertex(Collection<T> values) {
        for (T t : values) if (containsVertex(t)) return true;
        return false;
    }

    void addVertex(T value);

    default void addAllVertices(Collection<T> values) {
        for (T value : values) addVertex(value);
    }
    ...
    default boolean searchDFS(T vertex, T searched) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (used.contains(current)) continue;
            used.add(current);
            if (current.equals(searched)) return true;
